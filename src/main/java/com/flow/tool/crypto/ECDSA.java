package com.flow.tool.crypto;

import com.flow.tool.Crypto;
...
//        BCECPublicKey bcecPublicKey = publicKey(publicKey);
    ...
    public byte[] removeZeroByte(byte[] bytes) {
        int i;
        ...
        if (i == 0) {
            ...
            if (bytes.length - i >= 0) System.arraycopy(bytes, i, result, 0, bytes.length - i);
            return result;
        }
    }
